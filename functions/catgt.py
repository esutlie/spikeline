...
def sort_files(session_list, file_paths):
    mouse_ids = []
        ...
        for bot_row in np.unique(bot_rows):
            ...
            if not os.exists(merge_path):
                ...
                if all_files:
                    ...
                    combined_catgt(paths, os.path.join(file_paths['external_path'], f'catgt_merge_{mouse}_bot{bot_row}'))
...
if __name__ == '__main__':
    ...
    # path = os.path.join(file_paths['external_path'], 'ES026_2022-01-24_bot170_0_g0')
    # catgt(path, path)
    session_list, file_list = generate_file_lists(file_paths=file_paths)
    sort_files(session_list, file_paths)
    # for session in session_list['external_path']:
